...
2. overall_soundscape: 英文环境音描述（纸艺定格动画风：纸张剪裁轻响、纸片翻动沙沙声、胶水涂抹声、安静书房的暖灯氛围、极轻的机械运转声）
...
5. POV 段（标记为第一人称）：开头写 "A first-person desktop POV over a paper-craft workbench, the narrator's own hands visible in frame holding scissors and paper pieces"，强调从叙述者视角看手部操作，无脸无全身
6. 非 POV 段：写 "No person appears in this shot"
7. 复古纸质拼贴定格动画风：vintage handmade paper collage stop-motion style, aged paper / cardboard / kraft paper materials, rough hand-cut edges, hard-edged paper shadows, paper grain texture, scissors / glue / ruler paper-craft tools, desktop diorama scenes, stop-motion hand-pushed animation, warm paper tones (aged yellow / kraft brown / small high-saturation accents), symbolic elements (paper-cut accelerator pipes, folded paper medicine vials, paper-collage atomic orbits). NO photorealistic, NO 3D render, NO flat illustration, NO cyberpunk, NO neon
8. 输出严格三字段格式，字段间空一行"""
...
def enforce_constraints(prompt, is_pov):
    ...
    if is_pov and "first-person desktop pov" not in p.lower():
        issues.append("POV 段缺第一人称标记")
    if "paper collage" not in p.lower() and "paper-craft" not in p.lower():
        issues.append("缺 paper collage 风格标记")
    return p, issues
