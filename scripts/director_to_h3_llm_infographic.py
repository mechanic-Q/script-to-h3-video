...
2. overall_soundscape: 英文环境音描述（数据信息图环境音：轻柔的数据提示音、图表翻页声、数字滚动声、按钮点击声、淡雅科技氛围底噪）
...
5. POV 段（标记为第一人称）：开头写 "A first-person desktop POV over a data dashboard, the narrator's own hands visible in frame pointing at charts and icons"，强调从叙述者视角看手部操作，无脸无全身
6. 非 POV 段：写 "No person appears in this shot"
7. 数据信息图图标展示风：dynamic infographic icon display style, deep blue / navy data dashboard background, glowing accent digits and charts (bar chart / line chart / donut chart / gauge), flat minimalist icons (glasses / phone / robot / chip / speaker), dynamic data visualization (numbers rolling, bars growing, bubbles popping, data cards flipping), clear information hierarchy, numbers prominent and readable. NO photorealistic, NO live-action, NO documentary, NO paper collage, NO anime
8. 输出严格三字段格式，字段间空一行"""
...
def enforce_constraints(prompt, is_pov):
    ...
    if "infographic" not in p.lower() and "data dashboard" not in p.lower():
        issues.append("缺 infographic 风格标记")
    return p, issues
