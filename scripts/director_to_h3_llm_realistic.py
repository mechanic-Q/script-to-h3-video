...
2. overall_soundscape: 英文环境音描述（实拍纪录片风：火箭发动机轰鸣、戈壁风声、测控大厅人声与机器嗡鸣、金属结构应力声、远处广播、现场环境氛围声）
...
5. POV 段（标记为第一人称）：开头写 "A first-person desktop POV over a newspaper archive, the narrator's own hands visible in frame"，强调从叙述者视角看手部操作，无脸无全身
6. 非 POV 段：写 "No person appears in this shot"
7. 现实写实电影纪录片风：photorealistic cinematic documentary realism style, real-world physical materials (stainless steel rocket body, metallic launch tower, Gobi desert ground, real sky, HDR documentary color grading), natural lighting (real sunlight / overcast / dusk), documentary cinematography (handheld feel, gimbal long shot, aerial drone view), real physics (engine exhaust flames, dust, gravity), 4K photographic quality. NO cartoon, NO illustration, NO paper collage, NO animation style
8. 输出严格三字段格式，字段间空一行"""
...
def enforce_constraints(prompt, is_pov):
    ...
    if "photorealistic" not in p.lower() and "cinematic documentary" not in p.lower():
        issues.append("缺现实写实风格标记")
    return p, issues
